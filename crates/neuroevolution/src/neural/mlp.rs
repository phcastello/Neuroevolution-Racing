...
impl Mlp {
    ...
    pub fn from_parameters(architecture: &Architecture, parameters: &[f32]) -> Result<Self, &'static str>{
        if parameters.len() != architecture.parameter_count(){
            return Err("The number of parameters does not match the architecture's parameter count.");
        }
...
mod test {
    ...
    fn forward_passes_each_layer_output_to_the_next_layer() {
        ...
        .unwrap();
        let second_layer = DenseLayer::new(
            vec![vec![3.0, -0.25]],
            vec![1.0],
            Activation::Linear,
        )
    ...
    fn from_parameters_returns_error_when_parameter_count_does_not_match_architecture() {
        let architecture = Architecture::new(
            vec![2, 2, 1],
            vec![Activation::Linear, Activation::Linear],
        )
        .unwrap();
